...
def supernode_generation(image, model_ft, device, Argument, save_dir):
    ...
    with tiff.TiffFile(image) as tif:
        ...
        tif_native_arr = tif.asarray(out='memmap')
    ...
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<                     
                ...
                if np.mean(cut_thresh) > 0.75:
                    ...
                    CutImage = tif_native_arr[filter_location[1]:filter_location[1]+imagesize, filter_location[0]:filter_location[0]+imagesize]
                    CutImage = Image.fromarray(CutImage)
...
def Parser_main():
    ...
    parser.add_argument("--database", default='NOVOMICS', help="Use in the savedir", type = str)
    parser.add_argument("--cancertype",default='GAST',help="cancer type",type=str)
    parser.add_argument("--graphdir",default="./Sample_data_for_demo/Graph_test/",help="graph save dir",type=str)
    parser.add_argument("--imagedir",default="./Sample_data_for_demo/Novomics_TIF_Raw_WSI/",help="svs file location",type=str)
